...
public class ParseTree {
	...
	public static boolean checkString(String input){
		...
		if(!checking.hasNext()) return false;
		...
		while(checking.hasNext()){
			...
			if (checking.hasNextDouble()){
				...
				if (current.equals("(")){
					...
					needsDouble = true;
					if(prev.equals("*")|| prev.equals("+")||prev.equals("-")||prev.equals("/")||prev.equals("^")||prev.equals("sin")||prev.equals("sqrt")||prev.equals("cos")||prev.equals("tan")||prev.equals("")){
					...
					if (prevIsDouble) return false;
					
				}
				else if(current.equals("!")){
					if(!prevIsDouble && !prev.equals(")") && !prev.equals("x") && !prev.equals("y") && !prev.equals("z")) return false;
				}
		...
		if (leftParens != rightParens) return false;
		return true;
	}
	...
	public void reduceExpression(){
		...
		while(!root.equalTo(reference)){
			...
			//need to run simplifyTree to possibly take care of extra terms we may have added in simplifyRational
		}
		root.removeExtraTerms();
		//sorts the tree into an order
		root.miniSort();
	}
	...
	private static boolean isOperator(String toCheck){
		if(toCheck.equals("+") ||toCheck.equals("-") ||toCheck.equals("*") ||toCheck.equals("/") ||toCheck.equals(")") ||toCheck.equals("(") || toCheck.equals("^") || toCheck.equals("!")) return true;
		return false;
	}
	...
	private class Node{
		...
		private Node(Node toCopy){
			this.numChildren = toCopy.numChildren;
		...
		private void increaseArraySize(int newSize, int prev){
			Node[] newArray = new Node[2 * newSize];
		...
		private void removeMinus(){
			if(value.equals("-") && numChildren == 1){
				value = "+";
				numChildren = 2;
				children[1] = children[0];
				children[0] = new Node("-1");
			}
			else if(value.equals("-") && numChildren == 2){
				value = "+";
			...
			//remove the minuses from children
			if(!isLeaf()){
		...
		private void levelTree(){
			if(value.equals("+")){
				//Separate variable so we can change numChildren if needed
				int toLoop = numChildren;
				for(int i = 0; i < toLoop; i++){
					//can we pull a level up?
					if(children[i].value.equals("+")){
						//do we need to increase the size of the array
						if(numChildren == children.length) increaseArraySize(numChildren + children[i].numChildren, numChildren);
						//transfer Nodes over, order does not matter for addition
						Node holder = new Node(children[i]);
						children[i] = holder.children[0];
						for(int j = 1; j < holder.numChildren; j++){
							children[numChildren + j - 1] = holder.children[j];
						}
						//update number of children
						numChildren += holder.numChildren - 1;
					}
				}
			}
		...
		private void simplifyRationals(){
			...
			if(value.equals("/") && (children[0].value.equals("0.0") || children[0].value.equals("0") || children[0].value.equals("-0.0") || children[0].value.equals("-0"))){
				value = "0";
				numChildren = 0;
				children = null;
			}
			...
			if(value.equals("/") && (children[1].value.equals("1") || children[1].value.equals("1.0"))){
				value = children[0].value;
				numChildren = children[0].numChildren;
			...
			if(value.equals("/")){
				...
				//simplify (a*x)/x and (a*x^b)/x
					...
					for(int i = 0; i < children[0].numChildren; i++){
					...
					if(children[1].isLeaf() && !children[1].isVariable() && children[0].children[0].isLeaf() && !children[0].children[0].isVariable()){
				...
				//simplify x/(a*x) and x/(a*x^b) and (a*x)/(b*x^c)
					...
					for(int i = 0; i < children[1].numChildren; i++){
					...
					//simplify a/(b * x)
					if(children[0].isLeaf() && !children[0].isVariable() && children[1].children[0].isLeaf() && !children[1].children[0].isVariable() && !children[0].value.equals("1") && !children[0].value.equals("1.0") && !children[0].value.equals("-1") && !children[0].value.equals("-1.0")){
				...
				//simplify things of the form a*x^b/(c*x^d) or a*x/b*x and things like that
					...
					for(int i = 0; i < children[0].numChildren; i++){
						for (int j = 0; j < children[1].numChildren; j++){
							//the two nodes are equal, no exponents, but not 1
							if(children[0].children[i].equalTo(children[1].children[j]) && !children[0].children[i].value.equals("1")){
							...
							else if(children[0].children[i].isLeaf() && !children[0].children[i].isVariable() && children[1].children[j].isLeaf() && !children[1].children[j].isVariable()){
			...
			if(value.equals("*")){
				for(int i = 0; i < numChildren; i++){
					...
					if(children[i].value.equals("/")){
						...
						children[i] = left;
						int currentNumChildren = numChildren;
						Node[] theChildren = children;
						value = "/";
						numChildren = 2;
						children = new Node[2];
						children[0] = new Node("*");
						children[0].numChildren = currentNumChildren;
		...
		private void removeExtraTerms(){
			...
			if(!isLeaf()){
				...
				if(value.equals("*")){
					int numDeleted = 0;
					for(int i = 0; i < numChildren; i++){
						if(children[i].value.equals("1") || children[i].value.equals("1.0")){
							//remove this node
							children[i].value = "deleteMe6";
							numDeleted++;
						}
						if(children[i].value.equals("0") || children[i].value.equals("-0.0") || children[i].value.equals("0.0") || children[i].value.equals("-0")){
					...
					if(numDeleted>0){
						Node[] newArray = new Node[numChildren-numDeleted];
						int m=0;
				...
				if(value.equals("^")){
					if(children[0].value.equals("0") || children[0].value.equals("0.0") || children[0].value.equals("-0.0")){
						numChildren = 0;
						value = "0";
					}
					if(children[0].value.equals("1") || children[0].value.equals("1.0")){
						numChildren = 0;
						value = "1";
					}
					if(children[1].value.equals("0") || children[1].value.equals("0.0") || children[1].value.equals("-0.0")){
						numChildren = 0;
						value = "1";
					}
					if(children[1].value.equals("1") || children[1].value.equals("1.0")){
						numChildren = children[0].numChildren;
						value = children[0].value;
						children = children[0].children;
					}
				...
				if(value.equals("+")){
					int numDeleted = 0;
					for(int i = 0; i < numChildren; i++){
						if(children[i].value.equals("0") || children[i].value.equals("0.0") || children[i].value.equals("-0.0")){
							//remove this node
			...
			if(value.equals("+")){
				for(int i = 0; i < numChildren;i++){
					for(int j = i + 1; j < numChildren; j++){
						if(children[i].value.equals("*") && children[j].value.equals("*") && children[i].numChildren == 2 && children[j].numChildren == 2){
							...
							if(children[j].value.equals("deleteMe")){
								Node[] newArray = new Node[numChildren-1];
								int m=0;
				...
				for(int i = 0; i < numChildren;i++){
					for(int j = i + 1; j < numChildren; j++){
						if(children[i].value.equals("^") && children[j].value.equals("^")){
							...
							//need to get rid of duplicate
							if(children[j].value.equals("deleteMe3")){
								Node[] newArray = new Node[numChildren-1];
								int m=0;
								for(int k = 0; k < numChildren; k++){
		...
		private void miniSort(){
			...
			if(numChildren >= 2 && (value.equals("+")||value.equals("*"))){
		...
		private boolean canSimplify(){
			...
			for(int i = 0; i < numChildren; i++){
				...
				if(children[i].isVariable()) return false;
			}
			return true;
		}
		...
		private void simplifyTree(){
			...
			else if(numChildren > 2 && value.equals("+")){
				double holder = 0;
				int numToCombine = -1;
				int combinedNumber = 0;
				for(int i=0;i<numChildren;i++){
					if(!children[i].isVariable() && children[i].isLeaf() && numToCombine != -1){
					...
					else if(!children[i].isVariable() && children[i].isLeaf()){
						if(numToCombine == -1) numToCombine = i;
						holder = Double.parseDouble(children[i].value);
					}
				}
				if(combinedNumber != 0){
					numChildren -= combinedNumber;
					Node[] newArray = new Node[numChildren];
					int j = 0;
					for (int i = 0; i < numChildren + combinedNumber; i++){
	    ...
	    public String toString(){
	    	...
	    	if (!isLeaf()){
	    		if(numChildren > 1){
	    			...
	    			toReturn.append(children[numChildren - 1] + " ");
	    			toReturn.append(" ) ");
	    		}
	    ...
	    private Node takeDerivitive(){
	    	//derivitive of constant is 0
	    	if(isLeaf() && !isVariable()) return new Node("0");
	    	//derivitive of a variable is 1
	    		...
	    		if (!children[1].isLeaf() || children[1].isVariable()) {
	    			Node toReturn = new Node(this);
	    			Node holder0 = new Node(children[0]);
	    			Node holder1 = new Node(children[1]);
	...
	public static void main(String[] args) {		
		ParseTree tester = new ParseTree("1 + x * 3");
		...
		if(!test1.toString().equals(" ( 3 +  ( -1 * 2  )   ) ")){
			System.out.println("failure for removeMinus");
		}
		//test for levelTree
		...
		if(!test9.toStringPrefix().equals("/ 1 * 4.0 x  ")){
		...
		if(!test14.toStringPrefix().equals("/ 1 * 2 ^ x 4.0   ")){
		...
		if(!test20.toStringPrefix().equals("/ 1 * 2 ^ x 2.0   ")){
